...
async function start(client: Whatsapp) {
  ...
  client.onMessage(async message => {
    try {
      ...
      if (message.mimetype) {
        ...
        console.log(
          'TCL: start -> message.from,message.body,message.id.toString()',
          message.from,
          message.body,
          message.id.toString()
        );
        ...
        console.log('TCL: start -> mensagem para quedma');
        sentMessageId = await client.sendText(
          '553499267092@c.us',
          message.body
        );
        console.log('start -> sentMessageId', sentMessageId);
  ...
  const groupCreationEvent = await client.createGroup(
    'coolnewgroup',
    '0000000000@c.us'
  );
